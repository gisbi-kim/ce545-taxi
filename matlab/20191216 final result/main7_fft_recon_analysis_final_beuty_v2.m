...
%% NOTE: change here!
target_pixel_loc = [53, 52]; % (row = y, col = x)


%% show loc 
figure(9); clf;
any_frame = 30; % arbitrary, but 30 is okay.
...
save_dir = strcat("place [", num2str(target_pixel_loc(1)), ", ", num2str(target_pixel_loc(2)), "]");
save_base_dir = fullfile('results', 'location analysis', save_dir);
save_path = fullfile(save_base_dir, 'place_location.png');
saveas(gcf, save_path);


%% Main 
TRAIN_DAYS = [1, 4, 7, 10, 14, 17, 21, 24, 28];
...
title_str = strcat("A reconstructed signal using frequencies top ", num2str(Topk), ...
    " (green: true, blue: reconstructed (pred), black vertical line: monday start time (00))");
title(title_str);
ylim(yrange);
xlim([1, num_original_datapoints]);
...
%
Topk = round((num_train_datapoints/2) * 0.9);
target_loc_pred = ... % last arg: num_freqs_to_use
    reconstructSignalUsingFreqs(target_pred_days, amplitude, freq_domain, phase, Topk); 
figure(10); 
subplot(num_subfigures, 1, idx_cur_cubfigure); idx_cur_cubfigure = idx_cur_cubfigure + 1;
...
for ii = 1 : frames
    save_dir = strcat("place [", num2str(target_pixel_loc(1)), ", ", num2str(target_pixel_loc(2)), "]");
    save_base_dir = fullfile('results', 'location analysis', save_dir, EVENT_TYPE);
    save_path = fullfile(save_base_dir, strcat('fft_recon_using_', num2str(TRAIN_DAYS(ii)), 'days.png'));
    imwrite(vid(:,:,:,ii), save_path);
end
